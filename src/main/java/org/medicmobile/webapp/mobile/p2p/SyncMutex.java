...
/**
	...
	*   — Server sync has priority over P2P
...
public class SyncMutex {
	...
	public synchronized boolean tryAcquire(SyncType type) {
		...
		// If a timed-out sync is lingering, force-release it
		if (isTimedOut()) {
	...
	/**
		* Server sync has priority. Returns true if the current P2P session
	...
	/**
		* Record activity to prevent timeout during long-running syncs.
	...
	/**
		* Check if the current sync has timed out due to inactivity.
		*/
