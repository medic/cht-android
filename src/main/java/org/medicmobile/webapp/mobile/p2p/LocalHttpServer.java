...
public class LocalHttpServer extends NanoHTTPD {
	...
	/**
		...
		*/
	@SuppressWarnings("java:S107") // Data class — groups server dependencies
	...
	private Response dispatchEndpoint(String endpoint, Method method, IHTTPSession session) {
		Response result = tryDispatchGet(endpoint, method);
		if (result != null) {
			return result;
		}
		result = tryDispatchPost(endpoint, method, session);
		if (result != null) {
			return result;
		}
	...
	private Response tryDispatchGet(String endpoint, Method method) {
		if (method != Method.GET) {
			return null;
		}
		if ("get-ids".equals(endpoint)) {
			return handleGetIds();
		}
		if ("get-deletes".equals(endpoint)) {
			return handleGetDeletes();
		}
		return null;
	}

	private Response tryDispatchPost(String endpoint, Method method, IHTTPSession session) {
		if (method != Method.POST) {
			return null;
		}
		if ("bulk-get".equals(endpoint)) {
			return handleBulkGet(session);
		}
		if ("accept-docs".equals(endpoint)) {
			return handleAcceptDocs(session);
		}
		if ("sync-complete".equals(endpoint)) {
	...
	private String readFromInputStream(IHTTPSession session) throws IOException {
		...
		int read;
		while (remaining > 0 && (read = session.getInputStream().read(buf, 0,
