...
/**
	...
	*
	* <p>Guard classify() MUST be deterministic — same input always produces same output.
...
public final class ScopeGuard {
	...
	public ValidationResult classify(JSONObject doc, ScopeManifest senderScope,
		...
		DocFields fields = new DocFields(
				doc.optString(KEY_DOC_ID, null),
				doc.optString("type", null),
				getParentId(doc));

		// Step 2: Verify sender scope
		ValidationResult senderCheck = checkSenderScope(fields, senderScope, parentIndex);
		if (senderCheck != null) {
			return senderCheck;
		}

		// Step 3: Classify against receiver scope
		return classifyForReceiver(fields, receiverScope, parentIndex);
	}

	/** Extracted doc fields used during classification. Reduces parameter passing. */
	static final class DocFields {
		final String docId;
		final String docType;
		final String parentId;

		DocFields(String docId, String docType, String parentId) {
			this.docId = docId;
	...
	private ValidationResult rejectInvalidDocs(JSONObject doc) {
		String docId = doc.optString(KEY_DOC_ID, null);
		if (docId == null || docId.isEmpty()) {
			return ValidationResult.reject("missing_id");
		}

		ValidationResult systemDocCheck = rejectSystemDoc(docId);
		if (systemDocCheck != null) {
			return systemDocCheck;
		}

		return rejectDisallowedContent(doc);
	}

	private ValidationResult rejectDisallowedContent(JSONObject doc) {
	...
	private ValidationResult rejectSystemDoc(String docId) {
	...
	private ValidationResult checkSenderScope(DocFields fields,
												ScopeManifest senderScope, Map<String, String> parentIndex) {
		if (isContactType(fields.docType)
				&& !isWithinFacilitySubtree(fields.docId, senderScope.getFacilitySubtreeRoot(), parentIndex)) {
			return ValidationResult.reject("sender_scope_violation: not in sender facility subtree");
		}
		if (TYPE_DATA_RECORD.equals(fields.docType)
				&& fields.parentId != null
				&& !isWithinFacilitySubtree(fields.parentId, senderScope.getFacilitySubtreeRoot(), parentIndex)) {
			return ValidationResult.reject("sender_scope_violation: report parent not in sender facility subtree");
		}
		return null;
	}

	private ValidationResult classifyForReceiver(DocFields fields,
													ScopeManifest receiverScope, Map<String, String> parentIndex) {
		if (isContactType(fields.docType)) {
			return classifyContact(fields.docId, receiverScope, parentIndex);
		}
		if (TYPE_DATA_RECORD.equals(fields.docType)) {
			return classifyDataRecord(fields.parentId, receiverScope, parentIndex);
		}
	...
	private String getParentId(JSONObject doc) {
		...
		return parent.optString(KEY_DOC_ID, null);
	}
